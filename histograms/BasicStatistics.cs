...
namespace histograms
{
    ...
    {
        private int classes;
        public BasicStatistics(int classes)
        {
        ...
        /*public  List<double> GetTrustedInterval(double theta, double sigma, double quantile)
        {
        ...
        public double GetWalshMedian(List<double> sample, int N)
        {
            double walsh_median = 0.0;
            ...
            if (X.Count % 2 != 0)
                walsh_median = X[X.Count / 2 + 1];
            else
                walsh_median = X[X.Count / 2] + X[X.Count / 2 + 1] / 2;

            return walsh_median;
        }
        /// <returns>Усічене середнє.</returns>
        public double GetTruncatedMean(double koef, int N, List<double> sample)
        {
